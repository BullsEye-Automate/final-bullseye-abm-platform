...
export default function GraficoIndustria({ meetings }: { meetings: Meeting[] }) {
  const [selectedBar, setSelectedBar] = useState<{ industria: string; status: string } | null>(null);
  ...
  const filteredMeetings = useMemo(() => {
    ...
    return meetings.filter(
      (m) => m.industria === selectedBar.industria && m.realizado === selectedBar.status
    );
    ...
    if (active && payload && payload[0]) {
      ...
      return (
        <div className="bg-[#251762] border border-[#62E0D8] p-3 rounded-lg shadow-xl text-xs">
          <p className="font-semibold text-white">{data.industria}</p>
  ...
  const handleBarClick = (industria: string, status: string) => {
    setSelectedBar({ industria, status });
  };
            ...
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <Bar
                  ...
                  fill={COLORS.Si}
                  name="Realizado"
                  onClick={(data: any) => handleBarClick(data.industria, "Si")}
                ...
                <Bar
                  ...
                  fill={COLORS.No}
                  name="No Realizado"
                  onClick={(data: any) => handleBarClick(data.industria, "No")}
                  style={{ cursor: "pointer" }}
                />
                <Bar
                  dataKey="Pendiente"
                  ...
                  name="Pendiente"
                  onClick={(data: any) => handleBarClick(data.industria, "Pendiente")}
                  style={{ cursor: "pointer" }}
                />
                <Bar
                  dataKey="Reagendar"
                  ...
                  name="Reagendar"
                  onClick={(data: any) => handleBarClick(data.industria, "Reagendar")}
                  style={{ cursor: "pointer" }}
                />
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col">
            <div className="bg-gradient-to-r from-[#251762] to-[#3a2a7d] px-8 py-6 flex items-center justify-between">
              <div>
                ...
                <p className="text-[#62E0D8]/70 text-sm mt-1">Industria: {selectedBar.industria}</p>
              </div>

...
type SortKey = keyof ClienteMetrics;
type NumericKey = Exclude<SortKey, "cliente_id" | "cliente_nombre">;
type SortDir = "asc" | "desc";

interface TablaRankingClientesProps {
  data: ClienteMetrics[];
}
...
export default function TablaRankingClientes({ data }: TablaRankingClientesProps) {
  ...
  const totals = useMemo(() => {
    ...
    const sum = (key: keyof ClienteMetrics) => data.reduce((acc, cliente) => acc + (cliente[key] as number), 0);
    return {
    ...
    <div className={isFullscreen ? "overflow-auto h-full" : "overflow-auto max-h-[600px]"}>
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
          <tr>
            <th
              ...
              title="Cliente al que pertenece el número de Allo de la llamada, o la reunión."
            >
              <button onClick={() => toggleSort("cliente_nombre")} className="flex items-center">
                Cliente
                <SortIcon column="cliente_nombre" />
              </button>
            </th>
            {COLUMNS.map((col) => (
              <th
        ...
        <tbody className="divide-y divide-gray-100">
          {sortedData.map((cliente, idx) => (
            <tr key={cliente.cliente_id} className={idx % 2 === 0 ? "bg-white hover:bg-gray-50" : "bg-gray-50 hover:bg-gray-100"}>
              <td className="px-4 py-3 font-medium text-gray-900">{cliente.cliente_nombre}</td>
              {COLUMNS.map((col) => (
                <td key={col.key} className="px-4 py-3 text-right text-gray-700">
                  {PERCENT_KEYS.has(col.key) ? `${(cliente[col.key] as number).toFixed(1)}%` : cliente[col.key]}
                </td>
              ))}
            </tr>
          ))}
    ...
    return (
      <div className="fixed inset-0 z-50 bg-white flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
          <h2 className="font-semibold text-gray-900">Ranking Clientes</h2>
          <button

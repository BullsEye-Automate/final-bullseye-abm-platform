...
interface SdrMultiSelectProps {
  sdrs: SdrOption[];
  selected: string[];
  onChange: (ids: string[]) => void;
}

export default function SdrMultiSelect({ sdrs, selected, onChange }: SdrMultiSelectProps) {
  ...
  const filtered = query.trim()
    ? sdrs.filter((s) => s.sdr_nombre.toLowerCase().includes(query.toLowerCase()))
    : sdrs;
  ...
  const label =
    selected.length === 0
      ? "Todos los SDRs"
      : selected.length === 1
      ? sdrs.find((s) => s.sdr_id === selected[0])?.sdr_nombre || "1 seleccionado"
      : `${selected.length} SDRs seleccionados`;
        ...
        >
          ...
          <div className="max-h-64 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              filtered.map((sdr) => {
                const checked = selected.includes(sdr.sdr_id);
                return (
                  <label
                    key={sdr.sdr_id}
                  ...
                  >
                    <span className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="truncate text-gray-800">{sdr.sdr_nombre}</span>
                    </span>
                    <input
                      type="checkbox"
                      className="sr-only"
                      checked={checked}
                      onChange={() => toggleSdr(sdr.sdr_id)}
                    />
                  </label>
                );
              })

...
export default function IcpPage() {
  ...
  function downloadPdf() {
    ...
    const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${fileLabel} — ${clientLabel}</title><style>@page{size:A4;margin:20mm}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1a1040;line-height:1.55;font-size:11pt}header{border-bottom:2px solid #62E0D8;padding-bottom:14pt;margin-bottom:24pt}.brand{color:#251762;font-weight:700;font-size:18pt}.brand span{color:#62E0D8}.meta{color:#6B6884;font-size:10pt;margin-top:4pt}h1{font-size:20pt;color:#251762;margin:0 0 4pt}h2{font-size:13pt;color:#251762;border-bottom:1px solid #E5E2F0;padding-bottom:4pt;margin-top:22pt;margin-bottom:10pt;page-break-after:avoid}.field{margin-bottom:12pt;page-break-inside:avoid}.label{font-size:9pt;font-weight:600;color:#6B6884;text-transform:uppercase;letter-spacing:.5pt;margin-bottom:3pt}.value{white-space:pre-wrap}footer{margin-top:32pt;padding-top:12pt;border-top:1px solid #E5E2F0;font-size:9pt;color:#6B6884;text-align:center}</style></head><body><header><div class="brand">Bulls<span>Eye</span></div><div class="meta">${clientLabel} · ${today}</div></header><h1>${fileLabel}</h1><div>${
      (() => {
        const sectionRe = /-{10,}\n(.+?)\n-{10,}\n\n([\s\S]*?)(?=\n\n-{10,}|$)/g;
        ...
        while ((m = sectionRe.exec(content)) !== null) {
          ...
          const fields = m[2].trim().split(/\n\n+/).map((blk) => {
            const fm = blk.match(/^\[(.+?)\]\n([\s\S]+)$/);
            if (!fm) return "";
            return `<div class="field"><div class="label">${fm[1].replace(/</g,"&lt;")}</div><div class="value">${fm[2].replace(/</g,"&lt;")}</div></div>`;
          }).filter(Boolean).join("");
    ...
    }</div><footer>Documento generado por BullsEye · ${today}</footer></body></html>`;

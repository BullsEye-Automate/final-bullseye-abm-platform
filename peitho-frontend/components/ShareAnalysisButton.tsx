...
// Compartir el análisis/feedback post-reunión con el cliente externo
// (11-09-2026), pedido explícito del usuario — mismo patrón que
// ShareResearchButton.tsx: genera/reusa un link público de solo lectura para
// ESTA reunión (POST /api/meetings/:id/analysis/share, proxy hacia el
...
export default function ShareAnalysisButton({
  ...
  const url = token && typeof window !== "undefined" ? `${window.location.origin}/analisis-compartido/${token}` : null;

  async function handleShare() {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/meetings/${meetingId}/analysis/share`, { method: "POST" });
  ...
  async function handleRevoke() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/meetings/${meetingId}/analysis/share`, { method: "DELETE" });
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      <p className="text-[11px] text-gray-400">Solo puede ver el análisis de esta reunión — no necesita login.</p>
      {error && <p className="text-xs text-red-600">{error}</p>}
    </div>
  );
}

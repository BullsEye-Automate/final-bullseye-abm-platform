...
// Genera mensajes (si hace falta) y sube los contactos a la campaña real de
// Lemlist del cliente (con findEmail/verifyEmail/findPhone/linkedinEnrichment).
// Se llama directo desde código server-side (no por HTTP) para evitar que el
// middleware de auth de Next.js rechace la llamada — un fetch interno a
// /api/lemlist/push no lleva la cookie de sesión del usuario y siempre
// devolvía 401 "No autenticado".
export async function pushContactsToLemlist(
  db: SupabaseClient,
  params: { client_id: string; contact_ids?: string[]; force_regenerate?: boolean }
): Promise<{ status: number; result: LemlistPushResult | { error: string } }> {
  if (!params.client_id) {
    return { status: 400, result: { error: "Se requiere client_id" } };
  }

  const apiKey = await getLemlistApiKey(db, params.client_id);
  if (!apiKey) return { status: 500, result: { error: "LEMLIST_API_KEY no configurado" } };
  ...
  if (!config?.lemlist_campaign_id) {
    return { status: 400, result: { error: "No hay campaña configurada en Config. cliente" } };
  }
  ...
  if (params.contact_ids?.length) {
    ...
    q = q.is("lemlist_pushed_at", null).eq("client_id", params.client_id);
  }

  const { data: contacts, error: contactsError } = await q.limit(20);
  if (contactsError) return { status: 500, result: { error: contactsError.message } };
  if (!contacts?.length) return { status: 200, result: { pushed: 0, skipped: 0, generated: 0, errors: [], reason: "no_contacts" } };
  ...
  for (const contact of contacts) {
    ...
    if (needsMessages) {
      try {
        ...
        const { data: globalExamples } = await db.from("message_examples").select("*")
          .eq("client_id", params.client_id).is("segment_id", null)
          .order("created_at", { ascending: false }).limit(5);
    ...
    // Teléfono se enriquece vía Clay waterfall (mejor calidad LATAM) cuando
    // el contacto pasó por Clay. Si todavía no tiene teléfono — porque Clay
    // no lo encontró, o porque nunca pasó por Clay (ej. búsqueda manual,
    // que va directo a Lemlist sin Clay) — le pedimos a Lemlist que también
    // busque teléfono.
    const stillNoPhone = !contact.phone?.trim() && !(contact as any).phone_clay?.trim();
  ...
  return { status: 200, result: { pushed, skipped, generated, errors, reason: skipped > 0 && pushed === 0 ? "no_email" : undefined } };
}

...
- [ ] **Paso 2** — Ampliar la recolección de research pre-reunión en el backend (LinkedIn + señales, más allá de la búsqueda web básica del MVP original). **Requisito de diseño del usuario:** no debe correr automático — cada reunión futura debe tener un botón "Iniciar research" que dispara la recolección solo cuando el ejecutivo lo pide explícitamente. Motivo: no todas las reuniones agendadas son con un prospecto real, y correr búsqueda web/LinkedIn automáticamente en todas gastaría créditos de las APIs sin necesidad. Implica: un endpoint tipo `POST /meetings/:id/research` (o similar) en vez de un cron/job automático, y un botón en la UI del Módulo 1.
- [ ] **Paso 3** — UI del Módulo 1 (página de detalle con ese research, incluyendo el botón "Iniciar research" de arriba). Nota: hoy `/reuniones/[id]` para una reunión futura sin analizar solo muestra "todavía no hay análisis" — falta construir ahí (o en una vista separada) el contenido real de investigación de empresa/prospecto.

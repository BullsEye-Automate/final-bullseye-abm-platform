...
- [x] **Paso 1 — Esqueleto del frontend.** `peitho-frontend/` (Next.js 14 App Router + Tailwind + Supabase Auth, mismo patrón que `bullseye-abm-platform/middleware.ts`) con login y dos páginas (`/reuniones/futuras`, `/reuniones/pasadas`) que listan datos reales desde un nuevo endpoint `GET /meetings?scope=upcoming|past` en `peitho-backend`. Sin página de detalle todavía. Compiló y buildeó limpio (`npm run build`), pendiente de que el usuario lo pruebe con sus propias credenciales de Supabase.
...
- [ ] **Paso 3** — UI del Módulo 1 (página de detalle con ese research).
- [ ] **Paso 4** — Agregar el puntaje 1-10 al prompt/esquema de análisis post-reunión.
- [ ] **Paso 5** — UI del Módulo 2 (página de detalle tipo DIIO con el análisis y el puntaje).

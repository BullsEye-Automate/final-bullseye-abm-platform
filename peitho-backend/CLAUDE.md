...
- [~] **Tarea 5** — Pipeline de análisis post-reunión (Deepgram + prompt de `peitho_prompt_analisis_v1.md` vía Anthropic API → `meetings.analysis`). Código implementado (`src/postMeetingAnalysis.ts`, `src/prompts/analisisPostReunion.ts`), se dispara solo en segundo plano desde `POST /meetings/:id/audio`. **Pendiente que el usuario la pruebe** con audio real (no silencio) y sus API keys de Deepgram/Anthropic configuradas (ver `README.md` sección Tarea 5).

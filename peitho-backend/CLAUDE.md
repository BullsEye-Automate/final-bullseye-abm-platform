...
- [~] **Tarea 4** — Extensión de Chrome (Manifest V3, `chrome.tabCapture`) + `POST /meetings/:id/audio`. Código implementado (`peitho-chrome-extension/` — carpeta hermana de `peitho-backend/`, no es parte de este paquete npm — y `src/routes/meetings.ts` con multer para la subida). **Pendiente que el usuario la pruebe** (ver `peitho-chrome-extension/README.md` para cargarla en Chrome, y `peitho-backend/README.md` sección Tarea 4 para probar el endpoint solo con curl).

...
- [x] **Tarea 5** — Pipeline de análisis post-reunión (Deepgram + prompt de `peitho_prompt_analisis_v1.md` vía Anthropic API → `meetings.analysis`). Completada y confirmada end-to-end: audio real (grabado desde un segundo dispositivo hablando en la llamada) → transcripción con Deepgram → análisis con Claude → `meetings.analysis` con el JSON completo y `status=analyzed`. **Fix posterior pendiente de reconfirmar:** se detectó que `chrome.tabCapture` no capturaba el micrófono y dejaba al ejecutivo sin escuchar al otro participante — ver el bug documentado en "Decisiones y aprendizajes" más abajo. Implementado en `peitho-chrome-extension/` (mic capture + mezcla + reconexión a parlantes), pendiente de que el usuario lo pruebe con una conversación real de dos personas.
